...
import cn.dianedun.view.SelectLandeviceDialog;
...
public class VideoShowActivity_offline extends BaseActivity  implements View.OnClickListener, SurfaceHolder.Callback,
...
////        mRealPlayTouchListener.setSacaleRect(Constant.MAX_SCALE, 0, 0, realPlaySvlp.width, realPlaySvlp.height);
    ...
    private List<EZSADPDeviceInfo> mEZSADPDeviceList = new ArrayList<>();
    @Override
    ...
    private void updateCamera(final EZSADPDeviceInfo sadp_device_info) {
        if (!sadp_device_info.isActived()) {
            showToast("设备未激活");
        }else{
                ...
                public void run() {
                    try {
                        final EZLoginDeviceInfo ezLoginDeviceInfo = EZHCNetDeviceSDK.getInstance()
                                .loginDeviceWithUerName("admin", "QWErty789", sadp_device_info.getLocalIp(),
                                        sadp_device_info.getLocalPort());
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                stayPlay(ezLoginDeviceInfo);
                            }
                        });
    ...
    private void handleVoiceTalkFailed(ErrorInfo errorInfo) {
        ...
        switch (errorInfo.errorCode) {
            case ErrorCode.ERROR_TRANSF_DEVICE_TALKING:
                Utils.showToast(VideoShowActivity_offline.this, R.string.realplay_play_talkback_fail_ison);
                break;
            case ErrorCode.ERROR_TRANSF_DEVICE_PRIVACYON:
                Utils.showToast(VideoShowActivity_offline.this, R.string.realplay_play_talkback_fail_privacy);
                break;
            case ErrorCode.ERROR_TRANSF_DEVICE_OFFLINE:
                Utils.showToast(VideoShowActivity_offline.this, R.string.realplay_fail_device_not_exist);
                break;
            ...
            case ErrorCode.ERROR_TTS_HNADLE_TIMEOUT:
                Utils.showToast(VideoShowActivity_offline.this, R.string.realplay_play_talkback_request_timeout, errorInfo.errorCode);
                break;
            ...
            case ErrorCode.ERROR_CAS_AUDIO_SEND_ERROR:
                Utils.showToast(VideoShowActivity_offline.this, R.string.realplay_play_talkback_network_exception, errorInfo.errorCode);
                break;
            default:
                Utils.showToast(VideoShowActivity_offline.this, R.string.realplay_play_talkback_fail, errorInfo.errorCode);
                break;
        }
    }
...
//        mRealPlayFullTalkAnimBtn.setEnabled(true);
    ...
    private void handlePtzControlFail(Message msg) {
        ...
        switch (msg.arg1) {
            case ErrorCode.ERROR_CAS_PTZ_CONTROL_CALLING_PRESET_FAILED:// 正在调用预置点，键控动作无效
                Utils.showToast(VideoShowActivity_offline.this, R.string.camera_lens_too_busy, msg.arg1);
                break;
            case ErrorCode.ERROR_CAS_PTZ_PRESET_PRESETING_FAILE:// 当前正在调用预置点
                Utils.showToast(VideoShowActivity_offline.this, R.string.ptz_is_preseting, msg.arg1);
                break;
            ...
            case ErrorCode.ERROR_CAS_PTZ_CONTROL_TIMEOUT_CRUISE_TRACK_FAILED:// 键控动作超时(当前正在轨迹巡航)
                Utils.showToast(VideoShowActivity_offline.this, R.string.ptz_control_timeout_cruise_track_failed, msg.arg1);
                break;
            case ErrorCode.ERROR_CAS_PTZ_PRESET_INVALID_POSITION_FAILED:// 当前预置点信息无效
                Utils.showToast(VideoShowActivity_offline.this, R.string.ptz_preset_invalid_position_failed, msg.arg1);
                break;
            case ErrorCode.ERROR_CAS_PTZ_PRESET_CURRENT_POSITION_FAILED:// 该预置点已是当前位置
                Utils.showToast(VideoShowActivity_offline.this, R.string.ptz_preset_current_position_failed, msg.arg1);
                break;
            case ErrorCode.ERROR_CAS_PTZ_PRESET_SOUND_LOCALIZATION_FAILED:// 设备正在响应本次声源定位
                Utils.showToast(VideoShowActivity_offline.this, R.string.ptz_preset_sound_localization_failed, msg.arg1);
                break;
            ...
            case ErrorCode.ERROR_CAS_PTZ_MIRRORING_FAILED:// 设备正在镜像操作（设备镜像要几秒钟，防止频繁镜像操作）
                Utils.showToast(VideoShowActivity_offline.this, R.string.ptz_operation_too_frequently, msg.arg1);
                break;
            ...
            case ErrorCode.ERROR_CAS_PTZ_PRESET_EXCEED_MAXNUM_FAILED:// 当前预置点超过最大个数
                Utils.showToast(VideoShowActivity_offline.this, R.string.ptz_preset_exceed_maxnum_failed, msg.arg1);
                break;
            case ErrorCode.ERROR_CAS_PTZ_PRIVACYING_FAILED:// 设备处于隐私遮蔽状态（关闭了镜头，再去操作云台相关）
                Utils.showToast(VideoShowActivity_offline.this, R.string.ptz_privacying_failed, msg.arg1);
                break;
            case ErrorCode.ERROR_CAS_PTZ_TTSING_FAILED:// 设备处于语音对讲状态(区别以前的语音对讲错误码，云台单独列一个）
                Utils.showToast(VideoShowActivity_offline.this, R.string.ptz_mirroring_failed, msg.arg1);
                break;
            ...
            case ErrorCode.ERROR_CAS_PTZ_ROTATION_RIGHT_LIMIT_FAILED:// 设备云台旋转到达右限位
                ...
                break;
            default:
                Utils.showToast(VideoShowActivity_offline.this, R.string.ptz_operation_failed, msg.arg1);
                break;
        }
    }
    ...
    private void handleSetVedioModeFail(int errorCode) {
        ...
        Utils.showToast(VideoShowActivity_offline.this, R.string.realplay_set_vediomode_fail, errorCode);
    }
    ...
    protected void bindEvent() {
        ...
        mLLlight.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mStatusLight = true;
                Toast.makeText(VideoShowActivity_offline.this, "正在灯控操作中", Toast.LENGTH_LONG).show();
                ViseApi api = new ViseApi.Builder(VideoShowActivity_offline.this).build();
...
//                        setRealPlayFailUI("暂无设备在线");
        ...
        mLLPlayBack.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mDeviceInfo == null)
                    return;
                //回放
                DateTimeDialog dateTimeDialog = new DateTimeDialog(VideoShowActivity_offline.this, null, VideoShowActivity_offline.this, 0);
                dateTimeDialog.show();
            }
        });
        mImgBack.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                AppManager.getInstance().finishActivity(VideoShowActivity_offline.this);
            }
        });
        ...
        mLLStream.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                new AlertView("选择码流", null,"取消", null,
                        new String[]{"高清","标清","流畅"},
                        VideoShowActivity_offline.this, AlertView.Style.ActionSheet, new OnItemClickListener() {
                    @Override
    ...
    private void onRecordBtnClick() {
        ...
        if (!SDCardUtil.isSDCardUseable()) {
            // 提示SD卡不可用
            Utils.showToast(VideoShowActivity_offline.this, R.string.remoteplayback_SDCard_disable_use);
            return;
        }

        if (SDCardUtil.getSDCardRemainSize() < SDCardUtil.PIC_MIN_MEM_SPACE) {
            // 提示内存不足
            Utils.showToast(VideoShowActivity_offline.this, R.string.remoteplayback_record_fail_for_memory);
            return;
        }
...
//        Toast.makeText(VideoShowActivity.this, "已将录像保存至目录：" + recordFilePath, Toast.LENGTH_SHORT).show();
    ...
    private void handleRecordFail() {
        Utils.showToast(VideoShowActivity_offline.this, R.string.remoteplayback_record_fail);
        if (mIsRecording) {
            stopRealPlayRecord();
        }
    }
    ...
    private void stopRealPlayRecord() {
        ...
        Toast.makeText(VideoShowActivity_offline.this, mCurRecordName, Toast.LENGTH_SHORT).show();
        mCurRecordName = "";
    ...
    private void setQualityMode(final EZConstants.EZVideoLevel mode) {
        // 检查网络是否可用
        if (!ConnectionDetector.isNetworkAvailable(VideoShowActivity_offline.this)) {
            // 提示没有连接网络
            Utils.showToast(VideoShowActivity_offline.this, R.string.realplay_set_fail_network);
            return;
        }
    ...
    private void onCapturePicBtnClick() {
        ...
        if (SDCardUtil.getSDCardRemainSize() < SDCardUtil.PIC_MIN_MEM_SPACE) {
            // 提示内存不足
            Utils.showToast(VideoShowActivity_offline.this, R.string.remoteplayback_capture_fail_for_memory);
            return;
        }
                ...
                public void run() {
                    ...
                    if (bmp != null) {
                        try {
                            ...
                            MediaScanner mMediaScanner = new MediaScanner(VideoShowActivity_offline.this);
                            mMediaScanner.scanFile(path, "jpg");
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    Toast.makeText(VideoShowActivity_offline.this, getResources().getString(R.string.already_saved_to_volume)+path, Toast.LENGTH_SHORT).show();
                                }
                            });
...
//            if (VideoShowActivity_offline.this.isFinishing()) {
//                return null;
//            }
//
//            if (!ConnectionDetector.isNetworkAvailable(VideoShowActivity_offline.this)) {
...
//                    showToast("已过期，请重新登录");
//                    startActivity(new Intent(VideoShowActivity_offline.this,LoginActivity.class));
//                    break;
//                default:
